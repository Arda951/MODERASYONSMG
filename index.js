    ...
    client.on('message', async msg => { 
      ...
      client.on('message', async msg => { 
        ...
        client.on('message', async msg => { 
          ...
          client.on('message', async msg => { 
            if (msg.content.toLowerCase() === 'Selamın Aleykumm') { 
            await msg.react('🇦'); 
            msg.react('🇸'); 
            } 
            });
...
client.on("message", m => {
  if (m.channel.id !== "779331417838452764") {  //Sadece Resim Atılacak Kanal İDSİ
    return;
  }
...
client.on(`guildMemberAdd`, async member => {
  ...
  client.channels.get("").send(maze) //Maze yaptı çalanı lucifer yakar, sağlığınız zarar görebilir ^^
  });